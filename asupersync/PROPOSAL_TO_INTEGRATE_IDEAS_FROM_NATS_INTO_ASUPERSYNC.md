...
This is one of the most underexploited NATS ideas.
...
Cells, consumers, federation edges, and supervisor domains could emit local consistency facts. The fabric can then check whether they compose globally:
...
That is more than observability. It is a path toward distributed topology and coherence tooling that detects global incoherence before it turns into outages.
...
This is a genuinely different kind of scale-out story. The fabric stops asking only "where should this subject go?" and starts asking "what has to serialize, and what merely happens to share a textual prefix?" The point is not to discard canonical, hash-stable `SubjectCell` ownership; it is to avoid treating that ownership layer as the only concurrency boundary.
...
This only works where external side effects are either fenced off or modeled explicitly. The point is not magical time travel for arbitrary unmanaged systems; it is addressable branch semantics for the parts of the runtime Asupersync actually owns.
...
The phases below are one continuous plan. Subject cells, control capsules, delegated cursor partitions, and cell-scoped keying are first-class parts of the proposal, not a side track.
...
### Phase 8: NATS Edge Compatibility
...
Asupersync's combinators are not merely helpers; they come with explicit laws. That means a messaging workflow can be treated as a plan that is optimized subject to semantic side conditions.

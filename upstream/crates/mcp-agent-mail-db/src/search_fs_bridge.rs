...
#[must_use]
pub fn to_fs_config(config: &crate::search_two_tier::TwoTierConfig) -> FsTwoTierConfig {
    FsTwoTierConfig {
...
#[must_use]
pub fn from_fs_config(config: &FsTwoTierConfig) -> crate::search_two_tier::TwoTierConfig {
    crate::search_two_tier::TwoTierConfig {
        quality_weight: config.quality_weight as f32,
        fast_only: config.fast_only,
        ..crate::search_two_tier::TwoTierConfig::default()
    }
}
...
#[must_use]
pub const fn from_fs_model_tier(tier: FsModelTier) -> crate::search_embedder::ModelTier {
    match tier {
        FsModelTier::Fast => crate::search_embedder::ModelTier::Fast,
        FsModelTier::Quality => crate::search_embedder::ModelTier::Quality,
    }
}
...
pub struct SyncEmbedderAdapter {
    inner: Arc<dyn crate::search_two_tier::TwoTierEmbedder>,
    model_name: String,
    is_semantic: bool,
...
impl SyncEmbedderAdapter {
    ...
    #[must_use]
    pub fn new(
        embedder: Arc<dyn crate::search_two_tier::TwoTierEmbedder>,
        is_semantic: bool,
        category: FsModelCategory,
    ) -> Self {
    ...
    #[must_use]
    pub fn fast(embedder: Arc<dyn crate::search_two_tier::TwoTierEmbedder>) -> Self {
        Self::new(embedder, true, FsModelCategory::StaticEmbedder)
    }

    /// Create a quality-tier adapter (e.g., for `FastEmbed`).
    #[must_use]
    pub fn quality(embedder: Arc<dyn crate::search_two_tier::TwoTierEmbedder>) -> Self {
        Self::new(embedder, true, FsModelCategory::TransformerEmbedder)
    }

    /// Create a hash-tier adapter.
    #[must_use]
    pub fn hash(embedder: Arc<dyn crate::search_two_tier::TwoTierEmbedder>) -> Self {
...
/// crates' error enums.
#[must_use]
pub fn map_fs_error(err: frankensearch::SearchError) -> crate::search_error::SearchError {
    match err {
        frankensearch::SearchError::ModelNotFound { name } => {
            crate::search_error::SearchError::ModeUnavailable(format!("model not found: {name}"))
        }
        ...
        other => crate::search_error::SearchError::Internal(other.to_string()),
    }
}
...
mod tests {
    ...
    #[test]
    fn config_roundtrip() {
        let config = crate::search_two_tier::TwoTierConfig {
            quality_weight: 0.8,
            fast_only: true,
            ..crate::search_two_tier::TwoTierConfig::default()
        };
    ...
    fn model_tier_conversion() {
        assert_eq!(
            to_fs_model_tier(crate::search_embedder::ModelTier::Hash),
            FsModelTier::Fast
        );
        assert_eq!(
            to_fs_model_tier(crate::search_embedder::ModelTier::Fast),
            FsModelTier::Fast
        );
        assert_eq!(
            to_fs_model_tier(crate::search_embedder::ModelTier::Quality),
            FsModelTier::Quality
        );
        assert_eq!(
            from_fs_model_tier(FsModelTier::Fast),
            crate::search_embedder::ModelTier::Fast
        );
        assert_eq!(
            from_fs_model_tier(FsModelTier::Quality),
            crate::search_embedder::ModelTier::Quality
        );
    }
    ...
    fn map_fs_error_model_not_found() {
        ...
        match mapped {
            crate::search_error::SearchError::ModeUnavailable(msg) => {
    ...
    fn map_fs_error_cancelled() {
        ...
        let mapped = map_fs_error(err);
        match mapped {
            crate::search_error::SearchError::Timeout(msg) => {
    ...
    impl crate::search_two_tier::TwoTierEmbedder for StubEmbedder {

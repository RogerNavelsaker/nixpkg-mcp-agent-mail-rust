...
pub trait Embedder: Send + Sync {
    ...
    ///
    /// The text should already be canonicalized (via [`crate::canonical::canonicalize`]).
    ///
    /// # Errors
...
impl Embedder for HashEmbedder {
    fn embed(&self, text: &str) -> SearchResult<EmbeddingResult> {
        let hash = crate::canonical::content_hash(text);
    ...
    fn embed_batch(&self, texts: &[&str]) -> SearchResult<Vec<EmbeddingResult>> {
        Ok(texts
            .iter()
            .map(|t| {
                let hash = crate::canonical::content_hash(t);
